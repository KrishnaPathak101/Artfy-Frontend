...
const Yourwork = () => {
  ...
  useEffect(() => {
    if (user && isLoaded) {
      const getArts = async () => {
        try {
          const response = await axios.get(`https://artify-backend-gk1b.onrender.com/api/user/${user.id}`);
  ...
  const handleDelete = async ( id) => {
    ...
    try {
      await axios.delete(`https://artify-backend-gk1b.onrender.com/api/art/${id}`);
